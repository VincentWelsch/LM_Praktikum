...
async def get_fixes(runId: str):
    ...
    # answer = {"success": 0, "message": "Error", "result": {}}
    try:
        cur = conn.cursor()
        cur.execute("""SELECT longitude, latitude, altitude
                        FROM fix
                        WHERE runId = ?
                    """,
                    (runId,),
        ...
        rows = cur.fetchall()
        if not rows:
            return {"success": 0, "message": f"No fixes for the ID {runId} found"}
        else:
            fixes = [
                {"longitude": r[0], "latitude": r[1], "altitude": r[2]}
                for r in rows
            ]
            return {"success": 1, "fixes": fixes}
    except Exception as e:
        print("Error:", e)
        return {"success": 0, "message": "SQL error"}
    finally:
        conn.close()
    return answer
